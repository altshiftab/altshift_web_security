...
func Analyze(headerValue string) []*sarif.Result {
	...
	if headerValue == "unsafe-none" {
		return []*sarif.Result{
			httpHeadersSecurityCheckerInternal.MakeRuleIdResult(rule_id.CrossOriginEmbedderPolicyUnsafeNone),
		}
	}

	if headerValue != "require-corp" && headerValue != "credentialless" {
		return []*sarif.Result{
			httpHeadersSecurityCheckerInternal.MakeRuleIdResult(rule_id.CrossOriginEmbedderPolicyBadValue),
		}
	}
